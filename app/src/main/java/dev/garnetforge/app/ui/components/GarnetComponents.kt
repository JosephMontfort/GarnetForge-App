...
// ── Core frequency grid ──────────────────────────────────────────────
@Composable
fun CoreFreqGrid(cores: List<Int>, freqs: List<Int>, maxFreq: Int, color: Color) {
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
        cores.forEach { coreIdx ->
            val mhz = freqs.getOrElse(coreIdx) { 0 }
            val frac = if (maxFreq > 0) (mhz.toFloat() / maxFreq).coerceIn(0f, 1f) else 0f
...
fun ChipRowTuning(
    ...
    Column {
        ...
        Row(
            ...
            options.forEach { opt ->
                ProfileChip(opt, selected == opt) { onSelect(opt) }
            }
        }
    }
}
